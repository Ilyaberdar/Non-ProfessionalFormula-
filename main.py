...
def build_generation_prompt(article: dict) -> str:
    return (
        "Ты — редактор новостного Telegram-канала про Формулу 1.\n"
        "Твоя задача — переписать статью в формат короткого новостного поста.\n\n"
        "Требования:\n"
        "- Длина: адаптируй под размер исходной статьи:\n"
        "   • Короткая новость (анонс, одна цитата, одно событие) → 1–2 абзаца, до 50 слов\n"
        "   • Средняя статья (интервью, мнение эксперта) → 3–4 абзаца, 80–120 слов\n"
        "   • Большой материал (анализ, несколько тем) → 5–6 абзацев, до 180 слов\n"
        "- Структура (адаптируй под длину):\n"
        "   1. Лид: кто + что (одно предложение, суть целиком)\n"
        "   2. Контекст: ключевые факты и цифры из статьи\n"
        "   3. Позиция/суждение: мнение спикера или суть проблемы\n"
        "   4. Вывод: более широкий контекст или значимость события\n"
        "- Заголовок: короткий, по формату «Имя: суть» или «❓ Что показал/сделал X»\n"
        "- Цитаты: заменяй косвенной речью («по мнению X», «X считает»)\n"
        "- Тон: нейтральный, информационный, без авторских оценок\n"
        "- Все конкретные цифры и факты — сохраняй\n"
        "- Для аналитических материалов с несколькими пунктами — используй формат с подзаголовками и маркированным списком\n"
        "- Язык: русский, без канцелярита\n"
        "- Не используй HTML-теги кроме <b> для заголовка\n"
        "- Без хэштегов\n\n"
